...
c     call geom_reset(1)  ! Recompute g1m1 etc. with deformed only
      ...
      mg_fld = 1
      call h1mg_index_0_acc      ! set indices to 0

      call h1mg_setup_mg_nx_acc
      call h1mg_setup_semhat_acc ! SEM hat matrices for each level
      call h1mg_setup_intp_acc   ! Interpolation operators
      call h1mg_setup_dssum_acc  ! set direct stiffness summation handles
      call h1mg_setup_wtmask_acc ! set restriction weight matrices

      call h1mg_setup_fdm_acc    ! set up fast diagonalization method
      call h1mg_setup_schwarz_wt_acc
      l=mg_h1_lmax
      call mg_set_msk_acc (p_msk,l)
      return
      end
c----------------------------------------------------------------------
      subroutine h1mg_index_0_acc ! initialize index sets
      include 'SIZE'
      include 'HSMG'
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_mg_nx_acc()
      include 'SIZE'
      include 'HSMG'
      integer nf,nc,nr
      integer nx,ny,nz
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_semhat_acc ! SEM hat matrices for each level
      include 'SIZE'
      include 'HSMG'
      include 'SEMHAT'
c     n+1 sized ah arrays for n =nx of level
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_intp_acc
      include 'SIZE'
      include 'HSMG'
      integer l,nf,nc
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_intpm_acc(jh,zf,zc,nf,nc)
      include 'SIZE'
      integer nf,nc
      real jh(nf,nc),zf(1),zc(1)
      real w(2*lx1+2)
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_wtmask_acc
      include 'SIZE'
      include 'HSMG'
      integer i,l
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_rstr_wt_acc(wt,nx,ny,nz,l,w)
      include 'SIZE'
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_fast_acc(s,d,nl,ah,bh,n)
      include 'SIZE'
      include 'PARALLEL'
      include 'HSMG'
...
c          write(6,*)
      ...
      do ie=1,nelt
         ...
         call h1mg_setup_fast1d_acc(s(1,1,1,ie),lr,nr,lbr,rbr
     $            ,llr(ie),lmr(ie),lrr(ie),ah,bh,n,ie)
         call h1mg_setup_fast1d_acc(s(1,1,2,ie),ls,ns,lbs,rbs
     $            ,lls(ie),lms(ie),lrs(ie),ah,bh,n,ie)
         call h1mg_setup_fast1d_acc(s(1,1,3,ie),lt,nt,lbt,rbt
     $                     ,llt(ie),lmt(ie),lrt(ie),ah,bh,n,ie)
         il=1
         eps = 1.e-5 * (vlmax(lr(2),nr-2)
     $               + vlmax(ls(2),ns-2) + vlmax(lt(2),nt-2))
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_fast1d_acc(
     $   s,lam,nl,lbc,rbc,ll,lm,lr,ah,bh,n,ie)
      integer nl,lbc,rbc,n
      real s(nl,nl,2),lam(nl),ll,lm,lr
      real ah(0:n,0:n),bh(0:n)
      ...
      call h1mg_setup_fast1d_a_acc(s,lbc,rbc,ll,lm,lr,ah,n)
      call h1mg_setup_fast1d_b_acc(b,lbc,rbc,ll,lm,lr,bh,n)

      call generalev_acc(s,b,lam,nl,w)
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_fast1d_a_acc(a,lbc,rbc,ll,lm,lr,ah,n)
      integer lbc,rbc,n
      real a(0:n+2,0:n+2),ll,lm,lr
      real ah(0:n,0:n)
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_fast1d_b_acc(b,lbc,rbc,ll,lm,lr,bh,n)
      integer lbc,rbc,n
      real b(0:n+2,0:n+2),ll,lm,lr
      real bh(0:n)
...
c----------------------------------------------------------------------
      subroutine generalev_acc(a,b,lam,n,w)
c     Solve the generalized eigenvalue problem  A x = lam B x
c
c     A -- symm.
c     B -- symm., pos. definite
...
c-----------------------------------------------------------------------
      subroutine row_zero_acc(a,m,n,e)
      integer m,n,e
      real a(m,n)
      do j=1,n
         a(e,j)=0.
...
c-----------------------------------------------------------------------
      subroutine get_bcs_acc(lbr,rbr,lbs,rbs,lbt,rbt,ie)
c     based on Dirichlet boundary conditions set on all 6faces
      include 'SIZE'
      include 'PARALLEL'
      integer lbr,rbr,lbs,rbs,lbt,rbt
...
c-----------------------------------------------------------------------
      subroutine h1mg_setup_schwarz_wt_acc
      include 'SIZE'
      include 'HSMG'
...
c    $          mg_schwarz_wt(mg_schwarz_wt_index(l,mg_fld))
c    $         ,nl,mg_worke)
         call h1mg_setup_schwarz_wt3d_1_acc(
     $          mg_schwarz_wt(mg_schwarz_wt_index(l,mg_fld))
     $         ,l)
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_schwarz_wt3d_1_acc(wt,l)
      include 'SIZE'
      include 'HSMG'
...
c     Sum overlap region (border excluded)
     ...
     $    mg_work(i),2,one,mg_work(i),0,one,enx,eny,enz)
      ...
      do ie=1,nelt
c        call outmat(mg_work(k),nx,ny,'NEW WT',ie)
         call h1mg_setup_schwarz_wt3d_2_acc(wt,ie,nx,mg_work(k))
         k = k+nxyz
      enddo
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_schwarz_wt3d_2_acc(wt,ie,n,work)
      include 'SIZE'
      integer n
      real wt(n,n,4,3,nelt)
      real work(n,n,n)
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_schwarz_wt3d_acc(wt,n,work)
      include 'SIZE'
      include 'PARALLEL'
      integer n
      real wt(n,n,4,3,nelt)
...
c----------------------------------------------------------------------
      subroutine mg_set_msk_acc(p_msk ,l0)
      include 'SIZE'
      include 'HSMG'
      integer p_msk
...
c----------------------------------------------------------------------
      subroutine h1mg_setup_mask_acc(mask,nm,nx,ny,nz,nel,l,w)
      include 'SIZE'
...
c     Assumes that preprocessing has been completed via h1mg_setup()
      ...
      integer p_msk
!$ACC DATA PRESENT(z,rhs,e_h,w_h,r_h)
      nel   = nelt
...
!Schwarz
      ...
      call copy_acc(r_h,rhs,n)                              ! r  := rhs
      do l = mg_h1_lmax-1,2,-1                        ! DOWNWARD Leg of V-cycle
         is = is + n
         n  = mg_h1_n(l,mg_fld)
                                                      !          T
         call h1mg_rstr_acc(r_h,l,.true.)                   ! r   :=  J r
!        OVERLAPPING Schwarz exchange and solve:
         call h1mg_schwarz_acc(e_h(is),r_h,sigma,l)           ! e := sigma W M       r
                                                      !  l            Schwarz l
      enddo
      is = is+n
      call h1mg_rstr_acc(r_h,1,.false.)                     ! r  :=  J  r
      p_msk = p_mg_msk(l,mg_fld)
      call h1mg_mask_acc(r_h,mg_imask(p_msk),nel)           !        -1
      call hsmg_coarse_solve_acc ( e_h(is) , r_h )            ! e  := A   r
      call h1mg_mask_acc(e_h(is),mg_imask(p_msk),nel)       !  1     1   1
c     nx = mg_nh(1)
c     call h1mg_mask(e(is),mg_imask(p_msk),nel)       !  1     1   1
c     call exitt
...
c-----------------------------------------------------------------------
      ...
      call h1mg_schwarz_wt_acc    (e,l)          ! e  := W^.5* e
      call h1mg_schwarz_part1_acc(e,r,l)        !  l           l
      call h1mg_schwarz_wt_acc    (e,l)          ! e  :=  e *W^.5
...
c-----------------------------------------------------------------------
      ...
      call h1mg_mask_acc  (r,mg_imask(pm),nelt)  ! Zero Dirichlet nodes
      call h1mg_schwarz_toext3d_acc (mg_work,r,mg_nh(l))
      enx=mg_nh(l)+2
      eny=mg_nh(l)+2
      enz=mg_nh(l)+2
      i = enx*eny*enz*nelt+1
...
c     exchange interior nodes
      call h1mg_extrude_acc (mg_work,0,zero,mg_work,2,one,enx,eny,enz)
      call h1mg_schwarz_dssum(mg_work,l)
      call h1mg_extrude_acc (mg_work,0,one ,mg_work,2,onem,enx,eny,enz)

      call h1mg_fdm_acc (mg_work(i),mg_work,l) ! Do the local solves

c     Sum overlap region (border excluded)
      call h1mg_extrude_acc(
     $    mg_work,0,zero,mg_work(i),0,one ,enx,eny,enz)
     ...
     $    mg_work(i),2,one,mg_work(i),0,one,enx,eny,enz)
      ...
      call h1mg_schwarz_toreg3d_acc(e,mg_work(i),mg_nh(l))
      call h1mg_dssum(e,l)                           ! sum border nodes
      call h1mg_mask_acc (e,mg_imask(pm),nelt) ! apply mask

      return
      end
c-----------------------------------------------------------------------
      subroutine h1mg_schwarz_wt_acc(e,l)
      include 'SIZE'
      include 'INPUT'
      include 'HSMG'
      call h1mg_schwarz_wt3d2_acc(
     $    e,mg_schwarz_wt(mg_schwarz_wt_index(l,mg_fld)),mg_nh(l))
      return
      end
c-----------------------------------------------------------------------
      subroutine h1mg_schwarz_wt3d2_acc(e,wt,n)
      include 'SIZE'
      integer n
      real e(n,n,n,nelt)
      real wt(n,n,4,3,nelt)
...
c-----------------------------------------------------------------------
      subroutine mg_mask_e_acc(w,mask) ! Zero out Dirichlet conditions
      include 'SIZE'
...
c-----------------------------------------------------------------------
      subroutine h1mg_schwarz_toreg3d_acc(b,a,n)
c     strip off ghost cell
      include 'SIZE'
      integer n
      real a(0:n+1,0:n+1,0:n+1,nelt),b(n,n,n,nelt)
...
c-----------------------------------------------------------------------
      subroutine h1mg_schwarz_toext3d_acc(a,b,n)
c     border nodes (ghost cell = 0)
      include 'SIZE'
      integer n
      real a(0:n+1,0:n+1,0:n+1,nelt),b(n,n,n,nelt)
...
c-----------------------------------------------------------------------
      subroutine h1mg_extrude_acc(arr1,l1,f1,arr2,l2,f2,nx,ny,nz)
      include 'SIZE'
      integer l1,l2,nx,ny,nz
      real arr1(nx,ny,nz,nelt),arr2(nx,ny,nz,nelt)
      real f1,f2
...
!$ACC LOOP COLLAPSE(2) 
      ...
      do i=1,nv
         ...
         do l=1,nu    ! serial loop, no reduction needed
            work(i-1+(j-1)*nv) = work(i-1+(j-1)*nv)+
...
!$ACC LOOP COLLAPSE(3)
     ...
     $       work2(i-1 + (j-1)*nv+k*nv*nv) +
     $       work(i-1 + (l-1)*nv+k*nv*nu)*s(l,j,1,2,ie)
            enddo
         enddo
         enddo
      enddo
...
!$ACC LOOP COLLAPSE(2)
      ...
      do i=1,nv*nv
         ...
         do l=1,nu    ! serial loop, no reduction needed
            e(i,j,ie) = e(i,j,ie) +
     $      work2(i-1 +(l-1)*nv*nv)*s(l,j,1,3,ie)
         enddo
!         r(i +(j-1)*nv*nv,1,ie) = d(i +(j-1)*nv*nv,ie)*e(i,j,ie)
      enddo
      enddo
!$ACC LOOP
      do i=1,nn
      ...
      do k = 0,nu-1
         ...
         do i=1,nv
            ...
            do l=1,nu    ! serial loop, no reduction needed
             work2(i-1 + (j-1)*nv+k*nv*nv) =
     $       work2(i-1 + (j-1)*nv+k*nv*nv) +
     $       work(i-1 + (l-1)*nv+k*nv*nu)*s(l,j,2,2,ie)
            enddo
         enddo
         enddo
      enddo
...
!$ACC LOOP COLLAPSE(2)
      ...
      do i=1,nv*nv
         ...
         do l=1,nu    ! serial loop, no reduction needed
            e(i,j,ie) = e(i,j,ie) +
     $      work2(i-1 +(l-1)*nv*nv)*s(l,j,2,3,ie)
         enddo
      enddo
      enddo
      enddo
...
c     computes
c     v = [C (x) B (x) A] u
      subroutine h1mg_tnsr3d_el_acc(v,nv,u,nu,A,Bt,Ct)
...
c-----------------------------------------------------------------------
      ...
      include 'HSMG'
      logical ifdssum
      real r(mg_nh(l+1),mg_nh(l+1),mg_nhz(l+1),nelt)
      integer l
      call h1mg_do_wt_acc(r,mg_rstr_wt(mg_rstr_wt_index(l+1,mg_fld))
     $                     ,mg_nh(l+1),mg_nh(l+1),mg_nhz(l+1))
      call h1mg_tnsr1_acc(r,mg_nh(l),mg_nh(l+1),mg_jht(1,l),mg_jh(1,l))
      if (ifdssum) call h1mg_dssum(r,l)
      return
      end
c-----------------------------------------------------------------------
c     u = wt .* u
      subroutine h1mg_do_wt_acc(u,wt,nx,ny,nz)
      include 'SIZE'
      integer nx,ny,nz
      real u(nx,ny,nz,nelt)
      real wt(nx,nz,2,ndim,nelt)
...
c-----------------------------------------------------------------------
      subroutine h1mg_tnsr1_acc(v,nv,nu,A,At)
c
c     v = [A (x) A (x) A] u
c
      integer nv,nu
      ...
      call h1mg_tnsr1_3d_acc(v,nv,nu,A,At,At)
...
c-----------------------------------------------------------------------
      subroutine h1mg_intp_acc(uf,uc,l) ! l is coarse level
      real uf(1),uc(1)
      integer l
      include 'SIZE'
      include 'HSMG'
...
c     v = [A (x) A] u      or
c     v = [A (x) A (x) A] u
      subroutine h1mg_tnsr_acc(v,nv,u,nu,A,At)
      integer nv,nu
      real v(1),u(1),A(1),At(1)

      call h1mg_tnsr3d_acc(v,nv,u,nu,A,At,At)
