...
!         call add2s1_acc(p,z,beta,n)                                     ! 2n
         call before_acc(z,r,c,p,rtz1,rtz2,beta,n)
         call ax_acc(w,p,g,ur,us,ut,wk,n)                                ! flopa
         call after_acc(w,c,p,x,r,rtz1,pap,rtr,n)
...
c-----------------------------------------------------------------------
      subroutine before_acc(z,r,c,p,rtz1,rtz2,beta,n)
      REAL z(n),r(n),c(n),p(n), work(1)
      real rtz1 = 0.
!$ACC DATA PRESENT(z(1:n),r(1:n),c(1:n),p(1:n))
!$ACC PARALLEL LOOP REDUCTION(+:rtz1)
      do i=1,n
         z(i)=r(i)
         rtz1 = rtz1 +r(i)*z(i)*c(i)
      enddo
!$ACC END PARALLEL LOOP
      call gop(rtz1,work,'+  ',1)
      beta = rtz1/rtz2
      if (iter.eq.1) beta=0.0
!$ACC PARALLEL LOOP
      do i=1,n
...
!$ACC END DATA
      ...
      END
c-----------------------------------------------------------------------
      subroutine after_acc(w,c,p,x,r,rtz1,pap,rtr,n)
      REAL w(n),x(n),r(n),c(n),p(n), work(1)
      pap = 0.
!$ACC DATA PRESENT(w(n),x(n),r(1:n),c(1:n),p(1:n))
...
!$ACC END PARALLEL LOOP
      ...
      alpha = rtz1/pap
      rtr = 0.
!$ACC PARALLEL LOOP REDUCTION(+:rtr)
      do i=1,n
        x(i) = x(i) + alpha*p(i)
        r(i) = r(i) - alpha*w(i)
        rtr = rtr +r(i)*r(i)*c(i)
